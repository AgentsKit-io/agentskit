...
```bash
npx -y @agentskit/mcp@0.3.6 --tools fetch,search
```
...
{
  "mcpServers": {
    "agentskit": { "command": "npx", "args": ["-y", "@agentskit/mcp@0.3.6", "--tools", "fetch,search"] }
  }
}
```
...
```bash
npx -y @agentskit/mcp@0.3.6 --agents legal-contract-reviewer,fintech-kyc-screener --provider openai
```
