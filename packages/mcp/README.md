...
```bash
npx -y @agentskit/mcp@0.4.5 --tools fetch,search
```
...
{
  "mcpServers": {
    "agentskit": { "command": "npx", "args": ["-y", "@agentskit/mcp@0.4.5", "--tools", "fetch,search"] }
  }
}
```
...
command = "npx"
args = ["-y", "@agentskit/mcp@0.4.5", "--tools", "fetch,search"]
```
...
```bash
npx @agentskit/mcp@0.4.5 --agents legal-contract-reviewer,fintech-kyc-screener --provider openai
```
...
Typed Registry projections use `fetchAgent` + `createTypedAgentTool`; their
