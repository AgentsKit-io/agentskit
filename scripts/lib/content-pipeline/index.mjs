...
export function evaluateRequiredGates(
  ...
  config,
  recipeId,
  { runCommands = true, commandCache = new Map() } = {},
) {
  ...
  return (config.requiredGates ?? []).map((gate) => {
    if (gate.mode === 'command') {
      ...
      const cacheKey = JSON.stringify(gate.command)
      let run = commandCache.get(cacheKey)
      if (run === undefined) {
        const [bin, ...args] = gate.command
        run = spawnSync(bin, args, { cwd: root, encoding: 'utf8', env: process.env, timeout: 120_000 })
        commandCache.set(cacheKey, run)
      }
      return run.status === 0
...
export function auditContentPipeline(root, { runExecutable = true, runRequiredGates = true } = {}) {
  ...
  for (const recipe of recipes) {
    try {
      const gateResults = evaluateRequiredGates(root, config, recipe.id, {
        runCommands: runRequiredGates,
        commandCache,
      })
      const atom = runPipeline(root, recipe.id, { runExecutable, gateResults })
      if (!runExecutable) failures.push(`${recipe.id}: executable verification was not run`)
      for (const gate of gateResults) {
        if (!gate.ok && config.requiredGates.find((item) => item.id === gate.id)?.mode === 'command') {
          failures.push(`${recipe.id}: ${gate.message}`)
        }
      }
      if (atom.publish.status === 'published') {
        failures.push(`${recipe.id}: publisher must never mark status published autonomously`)
      }
      if (!atom.variants?.docsPage || !atom.variants?.shortPost || !atom.variants?.thread) {
        failures.push(`${recipe.id}: atom missing required variants`)
      }
      if (!atom.storyboard) failures.push(`${recipe.id}: atom missing storyboard`)
      if (!atom.variants?.communityPost) failures.push(`${recipe.id}: atom missing community post`)
      if (!atom.variants?.example) failures.push(`${recipe.id}: atom missing executable example link`)

      const blocked = preparePublish(atom, { approved: false })
      if (blocked.status !== 'blocked') failures.push(`${recipe.id}: expected blocked publish without approval`)
      ...
      if (!threw) failures.push(`${recipe.id}: preparePublish must require approvedBy/approvedOn`)
    } catch (error) {
