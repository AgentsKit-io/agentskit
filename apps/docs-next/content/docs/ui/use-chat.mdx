---
title: useChat
description: The shared chat state and action contract adapted by each framework binding.
---

The `ChatReturn` type below is the canonical state/action contract every
AgentsKit UI binding adapts. It wraps `createChatController` from
`@agentskit/core` with the idioms of your host framework (React hook, Vue
composable, Svelte store, Solid signal, Angular service, React Native hook,
Ink hook). The returned object is not structurally identical in every host:
for example, Svelte exposes a `Readable<ChatState>` plus methods and Vue
