...
If you are coming from Vercel AI SDK, the least painful path is usually:

1. Keep your chat surface mental model.
2. Move the provider seam to `@agentskit/adapters`.
3. Replace your custom runtime glue with `@agentskit/runtime` only when you need multi-step behavior.
4. Add tools, memory, and observability incrementally.
...
- Tool `name` is explicit (it's an identity — see [ADR 0002](https://github.com/AgentsKit-io/agentskit/blob/main/docs/architecture/adrs/0002-tool-contract.md) T1).
...
**Also possible and not possible in Vercel AI SDK**: router and ensemble adapters. Coming in Phase 3 (#145, #146).
...
Memory load + save, retrieval per turn, tool resolution, error categorization, abort semantics — all handled by the runtime per [ADR 0006](https://github.com/AgentsKit-io/agentskit/blob/main/docs/architecture/adrs/0006-runtime-contract.md).
...
See [Recipe: Multi-agent research team](/docs/reference/recipes/research-team) for the full walkthrough.
...
- **You need `generateObject` with strict schema output** right now. AgentsKit will ship a structured-output primitive in Phase 2; today you'd handle it manually.
...
- [README — When you should NOT use AgentsKit](/docs/get-started/comparison) — the honest decision matrix
- [Concepts: Adapter](../concepts/adapter) — why this migration is mostly a rename
- [Recipe: Chat with RAG](/docs/reference/recipes/rag-chat) — the equivalent of Vercel AI SDK + LangChain stitched together, in 30 lines
- [Build your first agent](/docs/get-started/getting-started/build-your-first-agent)
- [Architecture at a glance](/docs/get-started/architecture-at-a-glance)
