...
| `openrouter`   | ✅        | ⚠️    | ⚠️         | ⚠️        | ⚠️    | ❌          | $–$$$ |
| `huggingface`  | ✅        | ❌    | ⚠️         | ❌        | ❌    | ❌          | $ |
| `ollama`       | ✅        | ❌ current adapter | ⚠️ (llava / vision model id) | ❌ | ❌ | ✅ | free |
| `lmstudio`     | ✅        | ⚠️    | ❌          | ❌        | ❌    | ✅          | free |
| `vllm`         | ✅        | ⚠️    | ❌          | ❌        | ❌    | ✅          | free |
| `llamacpp`     | ✅        | ⚠️    | ❌          | ❌        | ❌    | ✅          | free |
...
Legend: ✅ supported · ⚠️ depends on model / config · ❌ not exposed by the current adapter · `passthrough` = inherits whatever the wrapped runtime exposes. The table describes AgentsKit adapter behavior, not every capability a provider may expose outside this library. Cost tiers are relative ranges, not contractual prices — consult the provider for current rates.
