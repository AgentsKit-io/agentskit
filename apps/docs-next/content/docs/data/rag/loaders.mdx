...
docbridge:
  covers:
    - area:packages/rag/src/loaders
---

All loaders return `InputDocument[]` ready for `rag.ingest`. Remote requests and
body reads use a finite timeout and response-size limit by default; configure
`timeoutMs`, `maxResponseBytes`, and `signal` in loader options. `loadUrl` puts
the raw response body in `content` — no boilerplate stripping.
...
`docs[0].content` is the raw response text (HTML or otherwise). Pipe it into `rag.ingest` as-is, or clean it first. Only HTTPS origins explicitly listed in `allowedOrigins` are accepted, and redirects are rejected.
...
`@aws-sdk/client-s3` is an optional peer dependency. Node consumers can let the
Node entry resolve it lazily; browser, Expo/Metro, and React Native consumers
must pass command constructors explicitly. For Cloudflare R2 / MinIO, configure
the client's `endpoint` to the compatible host.
...
BYO parser (e.g. `pdf-parse`, `unpdf`) to keep core deps zero. Parser failures
are reported as `RagError` with `AK_RAG_LOAD_FAILED`.
