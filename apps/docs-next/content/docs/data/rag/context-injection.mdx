...
Pass `systemPrompt` to `createRuntime({ systemPrompt })` (or include it in the request context when calling a controller). Adapter factories configure providers; they do not accept prompt fields.
