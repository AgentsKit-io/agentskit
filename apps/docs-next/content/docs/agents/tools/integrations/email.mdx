...
```ts
import { email } from '@agentskit/tools/integrations'
...
Heavy drivers (nodemailer, imapflow, mailparser) are **not** bundled. Wrap whatever you already use — see the [trigger adapters recipe](/docs/reference/recipes/triggers-adapters) for reference nodemailer/imapflow wrappers.
