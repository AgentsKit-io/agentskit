...
For the long-running stream of normalised `CdcChangeEvent`s, use `createCdcStream(config)` — it yields an `AsyncIterable` meant for a long-running trigger or worker, not for the agent loop.
