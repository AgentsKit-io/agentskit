...
const deployTool = defineZodTool({
  name: 'deploy',
  description: 'Deploy to production',
  schema: z.object({ service: z.string() }),
  toJsonSchema: (schema) => zodToJsonSchema(schema),
...
For a ready-made confirmation surface, use [ToolConfirmation](/docs/ui/tool-confirmation). It renders calls with the `requires_confirmation` status and forwards approval or denial to the chat controller.
