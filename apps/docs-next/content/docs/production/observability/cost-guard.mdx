...
const runtime = createRuntime({
  adapter,
  observers: [costGuard({ budgetUsd: 0.50, controller: new AbortController() })],
})
```

When the budget is exceeded, the guard aborts the supplied controller. Pass the
same signal to the runtime so the run stops promptly.
