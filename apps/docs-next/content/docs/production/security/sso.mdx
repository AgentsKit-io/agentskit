...
// Validate the XML signature with the XML-DSig library selected by your app.
const assertion = await myXmlDsigValidator(samlResponseXml)
verifier.verifyClaims(assertion)
