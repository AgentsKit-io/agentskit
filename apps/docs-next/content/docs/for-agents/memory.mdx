...
- `createWebStorageMemory` from `@agentskit/memory/web-storage` — injected,
  SSR-safe `localStorage`/`sessionStorage` with runtime validation, byte/message
  bounds, cancellation, and host-owned legacy migration.
...
- `createVirtualizedMemory` (from `@agentskit/core`) — hot window + cold retriever.
