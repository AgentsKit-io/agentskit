...
Every `AgentEvent` may carry an optional `correlation` envelope. Set the stable
`operationId` at the host boundary and preserve local `runId`, `sessionId`,
`turnId`, `actionId`, and `traceId` values when they exist. Observability
propagates these values as span attributes without requiring a vendor SDK or a
shared runtime.
...
- `costGuard({ budgetUsd, controller, prices?, unknownModelPolicy?, onCost?, onExceeded?, onError? })` — per-run dollar ceiling; aborts via `AbortController` when exceeded and fails closed on unknown pricing by default.
- `multiTenantCostGuard({ budgets, defaultBudgetUsd?, tenantOf?, unknownModelPolicy?, … })` — per-tenant bookkeeping; host enforces (no automatic abort), and unknown pricing marks the tenant blocked by default.
- `createAdvancedCostGuard({ budgets, caps?, mode?, unknownModelPolicy?, disableRuntime?, alertSinks?, onError?, now? })` — production guard with rolling window caps, 50/80/100 % threshold + forecast alerts, and modes:
  - **`warn`** — observe only.
