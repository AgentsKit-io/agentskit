...
Serve AgentsKit tools to an MCP host that supports the documented `2024-11-05`
tools bridge. A thin bridge over `@agentskit/tools/mcp` (`createMcpServer` +
`createStdioTransport`).

## Install

```bash
npm install @agentskit/mcp@0.4.3
```
...
`agentskit-mcp` — `npx -y @agentskit/mcp@0.4.3 --tools fetch,search`. Expose whole agents
