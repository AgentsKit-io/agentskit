...
- Headless standalone components with Angular-specific inputs (`data-ak-*` only, **partial-Ivy AOT** via ng-packagr APF; not full React parity): `ChatContainerComponent`, `MessageComponent`, `InputBarComponent`, `MarkdownComponent`, `CodeBlockComponent`, `ToolCallViewComponent`, `ThinkingIndicatorComponent`, `ToolConfirmationComponent`.
...
// Inject an adapter backed by your server/edge endpoint. Browser env values are public.
...
- **Multiple chats**: the root service is a single-session boundary. For
  independent chats, provide `AgentskitChat` in each component's `providers`
  array so Angular creates one service/controller per subtree.
