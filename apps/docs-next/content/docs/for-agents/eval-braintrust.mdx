...
Remote case fields are excluded by default. Use `options.upload` to opt in to selected input, output, expected, and metadata fields; selected values are bounded by `maxFieldBytes` (default 4096).
