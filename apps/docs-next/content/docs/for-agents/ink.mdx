...
description: Terminal chat UI via Ink — shared `ChatReturn` state/actions with Ink-specific components, running in Node with ANSI theming and keyboard navigation.
docbridge:
  covers:
...
The package entry is ESM-only. CommonJS applications can load it with dynamic
`import('@agentskit/ink')`.
...
- **ANSI theming**: wrap the tree in `<InkThemeProvider theme={...}>`; `ChatContainer` and `ChatConfig` do not accept a theme prop.
