...
## Custom reranking (Cohere / BGE) + built-in BM25

`@agentskit/rag` exports Voyage, Jina, and BM25 rerankers. Cohere, BGE,
and other providers are custom `RerankFn` implementations, as shown below;
there are no `cohereReranker` or `bgeReranker` package exports.
...
Scores are min-max normalized to `[0, 1]` within the candidate set and the
non-negative relative weights are normalized before mixing. Invalid BM25
`k1` / `b` values fall back to documented defaults, so emitted scores stay
finite.
