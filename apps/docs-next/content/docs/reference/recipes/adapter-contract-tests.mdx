...
The shared suite runs against {counts.nativeAdapters} native adapters. The
current provider list is maintained by the adapter package and its generated
ecosystem statistics rather than duplicated in this guide.
