...
The shared suite runs against every native adapter in the package. The current
provider list is maintained by the adapter package rather than duplicated in
this guide.
