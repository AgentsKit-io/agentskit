...
| --- | --- | --- | --- |
| Codex CLI, IDE, desktop | `.codex/config.toml` | `codex mcp list` or `/mcp` | Official format + fixture |
| Claude Code | `.mcp.json` | `claude mcp list` or `/mcp` | Official format + fixture |
| Claude Desktop | `claude_desktop_config.json` | Settings → Developer | Official format + fixture |
| Cursor | `.cursor/mcp.json` | Settings → MCP | Official format + fixture |
| Cline | MCP Servers configuration UI | MCP Servers panel | Official format + fixture |
| Continue | `.continue/mcpServers/agentskit.yaml` | Agent mode tool list | Official format + fixture |

The fixtures assert the exact version-pinned wrappers. Codex and Cline encode
reviewable approval settings directly; the other hosts retain the approval and
trust behavior documented by their vendors.
...
Keep `autoApprove` empty so every MCP tool call remains reviewable. Cline's
current documentation describes more than one settings location across its IDE
and CLI surfaces, so use the configuration UI or `cline mcp` instead of assuming
a shared file path.
