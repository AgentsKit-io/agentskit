...
control whether calls prompt. Its current documentation describes more than one
settings location across IDE and CLI surfaces, so use the configuration UI or
`cline mcp` instead of assuming a shared file path.
