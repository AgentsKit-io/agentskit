...
description: Reference adapter snippets that wrap heavy drivers (nodemailer, imapflow, botbuilder, pg-logical-replication) into the driver-free contracts that trigger and polling layers consume.
---

The `@agentskit/tools` integrations stay driver-free by accepting small client adapters. Here are reference wrappers for the three integrations that event triggers most commonly consume.
...
Your trigger or poller owns the inbound loop (IMAP polling, Teams webhooks, a CDC
stream). Hand it the adapters above and give the agent the outbound tools:

```ts
import { email } from '@agentskit/tools'
import { emailTransport } from './email-adapter'

const outboundTools = [...email({ transport: emailTransport, defaultFrom: 'bot@example.com' })]
```
