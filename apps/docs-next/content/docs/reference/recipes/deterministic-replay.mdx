...
import { createRecordingAdapter } from '@agentskit/eval/replay'
import { saveCassette } from '@agentskit/eval/replay/io'
...
import { createReplayAdapter } from '@agentskit/eval/replay'
import { loadCassette } from '@agentskit/eval/replay/io'
import { createRuntime } from '@agentskit/runtime'
...
`@agentskit/eval/replay/io` is Node-only. Browser, Expo, and React Native applications can use `serializeCassette` and `parseCassette` from the universal replay entry, then delegate persistence to host storage.
