...
docbridge:
  ...
  relations:
    - from: package:@agentskit/adapters
...
The snapshot is large (thousands of models), so it ships **only** via the `./catalog` subpath — the main `@agentskit/adapters` bundle is unaffected. Provider and model counts are regenerated from the artifact rather than hand-maintained.
