...
description: Braintrust scoring pipeline for @agentskit/eval — quality + robustness scorers and CI regression alerts.
docbridge:
  covers:
    - package:@agentskit/eval
    - area:packages/eval-braintrust/src/scorers
  relations:
    - from: package:@agentskit/eval
    ...
    - from: package:@agentskit/eval
      ...
      kind: imports
      detection: static
    - from: package:@agentskit/eval-braintrust
      to: package:@agentskit/core
      kind: depends-on
      detection: static
    - from: package:@agentskit/eval-braintrust
      to: package:@agentskit/eval
      kind: depends-on
      detection: static
---
...
`@agentskit/eval/braintrust` connects AgentsKit eval suites to [Braintrust](https://www.braintrust.dev). It runs your test cases through deterministic scorers, optionally ships explicitly selected results to a Braintrust experiment, and surfaces regressions as CI annotations — all without requiring Braintrust to be installed at build time (peer-resolved at runtime). The implementation is a private workspace source package; consumers install the public `@agentskit/eval` package and its `/braintrust` subpath.

**Version:** `0.2.19`

## When to reach for it

- You already use Braintrust for experiment tracking and want AgentsKit runs to show up in its experiment UI.
...
| Export | Kind | Purpose |
|---|---|---|
| `runBraintrustEval(args, internals?)` | `async fn` | Run cases, score, optionally sync to Braintrust, return `ExperimentResult` |
...
| Env var | Purpose |
|---|---|
| `BRAINTRUST_API_KEY` | Explicitly enables Braintrust upload. Without it, scoring runs locally and no data is uploaded. |
...
Remote case fields are excluded by default. To upload selected fields, set `options.upload` explicitly with `includeInput`, `includeOutput`, `includeExpected`, or an allowlist of `metadataKeys`; every selected value is capped by `maxFieldBytes` (default 4096). Run-level `options.metadata` follows the same allowlist and is never sent by default.

## Stability

- **Version:** inherited from `@agentskit/eval`; the workspace adapter source is not published independently.
- **Tier:** beta
...
npm: [@agentskit/eval](https://www.npmjs.com/package/@agentskit/eval) · repo: [packages/eval-braintrust](https://github.com/AgentsKit-io/agentskit/tree/main/packages/eval-braintrust)
