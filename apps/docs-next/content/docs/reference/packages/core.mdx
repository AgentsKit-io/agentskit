...
The controller merges system prompts, runs retrieval, dispatches tool calls, saves via `ChatMemory` after successful turns, skips persistence for failed or aborted turns, surfaces background memory and skill activation failures through `onError`, and exposes subscribe/update patterns consumed by UI packages.
...
| `parseToolArgs` | Parse JSON tool arguments with `{ args, valid }` for fail-closed adapters |
| `safeParseArgs` | Backwards-compatible safe value-only parser |
...
| Export | Role |
|--------|------|
| `createInMemoryMemory`, `createLocalStorageMemory` | Simple bundled memories for tests or demos; malformed local-storage data raises `MemoryError` |
| `serializeMessages` / `deserializeMessages` | Persistence helpers |
| `validateMemoryRecord` from `@agentskit/core/memory-validation` | Bounded runtime validation for untrusted serialized messages |
...
| `@agentskit/core/tool-proposal` | Route a validated proposal through controller authorization |
| `createStaticRetriever`, `formatRetrievedDocuments` | Retriever helpers for static/demo context; matching is token substring overlap, not semantic production search |
...
| `MemoryError` | Thrown on memory load/save/clear/deserialize failures; links to memory docs |
...
ErrorCodes.AK_MEMORY_CLEAR_FAILED     // memory.clear() failed
