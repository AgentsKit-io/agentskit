...
The controller merges system prompts, runs retrieval, dispatches tool calls, saves via `ChatMemory` after successful turns, skips persistence for failed or aborted turns, and exposes subscribe/update patterns consumed by UI packages.
