...
```bash
npx @agentskit/cli add <agent> --run "<task>" --provider ollama
```
...
```bash
npx @agentskit/cli diff <agent>     # show how your copy differs from the registry
npx @agentskit/cli update <agent>   # apply the registry version
```
